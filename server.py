...
def declineaccess(client_name):
    ...
    monosodiumglutamate = f"oh oops {client_name} declined your request"
    
    other_client_socket.send(monosodiumglutamate.encode())
...
def grantaccess(client_name):
    ...
    monosodiumglutamate = f"oh yay {client_name} granted you access"
...
def handlesendfile(client_name, file_name, file_size):
    ...
    monosodiumglutamate = f"{client_name} wants to send file {file_size} with the file size of {file_size}. do you want to download????? Y/N"

    time.sleep(1)

    monosodiumglutamatedown = f"download: {file_name}"
    other_client_socket.send(monosodiumglutamatedown.encode())
...
def ftp():
    ...
    authorizer = authorizer.add_user("lftpd", "lftpd", ".", perm = "elradfmw")
